...
describe('markLoginAndNotifyPrevious', () => {
  ...
  test('does not arm the info flag when the prior entry is this same session', async () => {
    const registry = fakeRegistry(
      new Map([
        ['user-1', { lastLoginAt: 1000, lastLoginSessionId: 'sess-new' }]
      ])
    )
    const request = fakeRequest({ registry })
...
describe('concurrentLoginNoticeExt', () => {
  ...
  test('no-op for an unauthenticated request', async () => {
