...
import { _clearJwksCache, verifyDefraIdToken } from './defra-id-token.js'
...
describe('verifyDefraIdToken', () => {
  test('returns claims when signature, iss, aud, exp and nonce are valid', async () => {
    const { privateKey, jwk } = await setupKeys()
    mockJwks(jwk)
    const tok = await makeIdToken(privateKey, { sub: 'u1' })

    const claims = await verifyDefraIdToken(tok, {
      jwksUri,
      issuer,
      audience,
